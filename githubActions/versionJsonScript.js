        ...
        const currentReleases = await Promise.all(currentVersion["releases"].map(
            async (currentRelease, index) => {
                ...
                if (index == 0) {
                    ...
                    downloadUrls = await generateDownloadLinksObject(link, false , versionCode, currentVersion.name);
                ...
                // normal versions
                    ...
                    downloadUrls = await generateDownloadLinksObject(link, false,   versionCode, currentRelease.name);
                }
